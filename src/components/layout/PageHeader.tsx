    ...
    <header className="erp-page-header flex flex-col md:flex-row md:items-center justify-between gap-4">
      <div className="flex items-center gap-3 w-full md:w-auto">
        {showBack && (
          <button
            onClick={() => navigate(-1)}
            className="p-1.5 -ml-1 rounded-full hover:bg-gray-100 dark:hover:bg-slate-800 active:bg-gray-200 dark:active:bg-slate-700 transition-colors"
          >
        ...
        <div>
          ...
          {subtitle && <p className="text-[13px] text-gray-500 dark:text-gray-400 font-medium mt-1">{subtitle}</p>}
        </div>
      </div>
